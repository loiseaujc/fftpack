...
contains
   ...
   subroutine test_classic_dct(error)
      ...
      call dcosti(4, w)
      call dcost(4, x, w)
      call check(error, sum(abs(x - [real(dp) :: 15, -4, 0, -1.0000000000000009_dp])) < eps, &
                 "`dcost` failed.")
      ...
      call dcost(4, x, w)
      call check(error, sum(abs(x/(2*3) - [real(dp) :: 1, 2, 3, 4])) < eps, &
                 "2nd `dcost` failed.")
      if (allocated(error)) return
      ...
      call dcosqb(4, x, w)
      call check(error, sum(abs(x/(4*4) - [real(dp) :: 1, 2, 3, 4])) < eps, &
                 "`dcosqb` failed.")
      if (allocated(error)) return
   ...
   subroutine test_modernized_dct(error)
      ...
      if (allocated(error)) return
      call check(error, sum(abs(dct(x, 4, 1) - [real(dp) :: -3, -3.0000000000000036_dp, 15, 33])) < eps, &
                 "`dct(x,4,1)` failed.")
      !DCT-2
      x = [9, -9, 3]
