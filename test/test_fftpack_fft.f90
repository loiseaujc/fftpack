...
contains
   ...
   subroutine test_classic_fft(error)
      ...
      complex(dp) :: x(4) = [1, 2, 3, 4]
      real(dp) :: w(31)

      call zffti(4, w)
      call zfftf(4, x, w)
      call check(error, all(x == [complex(dp) ::(10, 0), (-2, 2), (-2, 0), (-2, -2)]), &
                 "`zfftf` failed.")
      if (allocated(error)) return
      call zfftb(4, x, w)
      call check(error, all(x/4.0_dp == [complex(dp) ::(1, 0), (2, 0), (3, 0), (4, 0)]), &
                 "`zfftb` failed.")
   ...
   subroutine test_modernized_fft(error)
      type(error_type), allocatable, intent(out) :: error
      real(dp) :: eps = 1.0e-10_dp
      complex(dp) :: x(3) = [1.0_dp, 2.0_dp, 3.0_dp]
   ...
   subroutine test_modernized_ifft(error)
      ...
      call check(error, sum(abs(ifft(fft(x))/4.0_dp - [complex(dp) :: 1, 2, 3, 4])) < eps, &
                 "`ifft(fft(x))/4.0_dp` failed.")
      if (allocated(error)) return
      call check(error, sum(abs(ifft(fft(x), 2) - [complex(dp) ::(8, 2), (12, -2)])) < eps, &
                 "`ifft(fft(x), 2)` failed.")
      if (allocated(error)) return
      call check(error, sum(abs(ifft(fft(x, 2), 4) - [complex(dp) ::(2, 0), (3, -1), (4, 0), (3, 1)])) < eps, &
